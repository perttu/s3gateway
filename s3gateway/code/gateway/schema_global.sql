-- Global S3 Gateway Database Schema
-- This database handles ONLY provider information and minimal routing
-- For compliance: ALL customer data (including compliance info) is stored in regional databases
...
-- Global routing log (for audit and debugging - minimal info only)
CREATE TABLE routing_log (
    id SERIAL PRIMARY KEY,
    customer_id VARCHAR(100),
    request_id UUID DEFAULT uuid_generate_v4(),
    source_ip INET,
    requested_endpoint VARCHAR(255),
    routed_to_region VARCHAR(50) REFERENCES regions(region_id),
    routed_to_endpoint VARCHAR(255),
    routing_reason VARCHAR(100), -- 'customer_region', 'compliance', 'load_balance'
...
('health_check_interval', '30', 'Provider health check interval in seconds');
