...
if [ "$QUIET" = false ]; then
    ...
    echo "Global Gateway:    http://0.0.0.0:8000 (with validation & redirects)"
    echo "FI-HEL Gateway:    http://0.0.0.0:8001 (standard validation)" 
    echo "DE-FRA Gateway:    http://0.0.0.0:8002 (strict validation)"
    echo "S3Proxy:          http://0.0.0.0:8080 (fallback)"
    echo ""
    echo -e "${BLUE}📝 Remote Access:${NC}"
    echo "Replace 0.0.0.0 with your server's IP address or hostname"
    echo "Example: http://your-server-ip:8000"
    echo ""
